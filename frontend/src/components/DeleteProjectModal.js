    ...
    const deleteProject = async () => {
        if (userInput === title) {
            try {
                ...
                await axios.delete(apiPath + "/project/delete/" + props.projectId, config).then((res) => {
                    if (res.status === 200) {
        ...
        >
            <div className="modal-dialog relative w-auto pointer-events-none">
                <div className="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-white bg-clip-padding rounded-md outline-none text-current">
                    ...
                    <p className='mt-3'>Are you sure you want to delete the project <span className='font-bold text-purple-500'>{props.item.title}</span> ?</p>
                        <p>The deletion of this information is irreversible, and will delete all related information(s). </p>
                        <p>Type <span className='text-red-500'>{props.item.title}</span> to authorize the operation:</p>
                    </div>

...
func main() {
	...
	//router.HandleFunc("/api/v1/user/{username}", controllers.User).Methods("GET")
	router.HandleFunc("/api/v1/user/create", controllers.CreateUser).Methods("POST")
	router.HandleFunc("/api/v1/user/update/{username}", controllers.UpdateUser).Methods("PUT")
	router.HandleFunc("/api/v1/user/delete/{username}", controllers.DeleteUser).Methods("DELETE")
	router.HandleFunc("/api/v1/user/validUsername/{username}", controllers.CheckUsernameExists).Methods("GET")
	router.HandleFunc("/api/v1/user/profile/{username}", controllers.UserProfile).Methods("GET")
	router.HandleFunc("/api/v1/user/lock/{username}", controllers.Lock).Methods("GET")
	router.HandleFunc("/api/v1/user/unlock/{username}", controllers.UnLock).Methods("GET")
	router.HandleFunc("/api/v1/user/locked", controllers.LockedUsers).Methods("GET")
	router.HandleFunc("/api/v1/user/role", controllers.GetUserRole).Methods("POST")
